...
#include "dll1/user_dll_1.h"
#include "dll2/user_dll_2.h"
...
DWORD WINAPI do_something ( LPVOID )
{
	...
	while (!g_Quit)
	{
		...
		foo::MathFuncs_1::Add(3.12415, 2.71828);
		//something_useful();
		Sleep(100);
	}
	return 0;
...
DWORD WINAPI do_something_2 ( LPVOID )
{
	...
	while (!g_Quit)
	{
		...
		++i;
		TRACE_MSG(trace::e_Info, trace::CTX_Default,  "Thread tick i=%u", i);
		foo::MathFuncs_1::Subtract(3.12415, 2.71828);
		//something_useful();
		Sleep(95);
	}
	return 0;
...
{
	printf("main()");
	TRACE_APPNAME("WarHorse_DLL_App");
	TRACE_CONNECT();
	...
	ThreadPool<2> thr_pool2;
	thr_pool2.Create(do_something_2, 0);
