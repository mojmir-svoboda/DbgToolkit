...
void ColorizerString::actionColor (QModelIndex const & sourceIndex, ColorizedString const & ct, QColor const & fg, QColor const & bg) const
{
...
void ColorizerString::actionUncolor (QModelIndex const & sourceIndex, ColorizedString const & ct) const
{
...
bool ColorizerString::action (QModelIndex const & idx)
{
	for (size_t i = 0, ie = m_data.size(); i < ie; ++i)
	{
		...
		actionColor(idx, ct, ct.m_fgcolor, ct.m_bgcolor);
	}
	return false;
}

bool ColorizerString::accept (QModelIndex const & idx)
{
	return true;
}
...
{
	//applyFnOnAllChildren([this] (QString const & item) { append(item); }, this, m_model, Qt::Checked);
	emitFilterChangedSignal();
}

void ColorizerString::onSelectNone ()
{
	//auto fn = &ColorizerString::remove;
