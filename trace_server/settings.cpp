...
void MainWindow::onSettingsIndexesMoved (QModelIndexList const & indexes)
{
}
...
void MainWindow::onSettingsAppSelected (int idx)
{
	...
	//connect(ui_settings->listViewColumnSetup, SIGNAL(indexesMoved(QModelIndexList const &)), this, SLOT(onSettingsIndexesMoved(QModelIndexList const &)));
...
void MainWindow::onSetup ()
{
	...
	QStandardItem * cs_root = model->invisibleRootItem();
	for (int i = 0, ie = m_columns_setup[idx].size(); i < ie; ++i)
	{
		//QList<QStandardItem *> row_items = addRow(m_columns_setup.at(idx).at(i), true);
		//cs_root->appendRow(row_items);
