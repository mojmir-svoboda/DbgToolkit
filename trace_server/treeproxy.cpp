...
void TreeProxyModel::force_update ()
{
	emit layoutAboutToBeChanged();
	QAbstractItemModel const * src_model = static_cast<QAbstractItemModel const *>(sourceModel());
...
QVariant TreeProxyModel::data (QModelIndex const & index, int role) const
{
	QModelIndex const src_idx = mapToSource(index);
	return sourceModel()->data(src_idx, role);
}

void TreeProxyModel::insertCol (int c)
{
	int pos0 = 0;
...
void TreeProxyModel::insertRow (int c)
{
	int pos0 = 0;
...
bool TreeProxyModel::setData (QModelIndex const & src_index, QVariant const & value, int role)
{
...
/*bool  TreeProxyModel::setHeaderData (int section, Qt::Orientation orientation, QVariant const & value, int role)
{
	int const tgt_idx = colFromSource(section);
...
Qt::ItemFlags TreeProxyModel::flags (QModelIndex const & index) const
{
	return sourceModel()->flags(index) | Qt::ItemIsEnabled | Qt::ItemIsSelectable;
}

QModelIndex TreeProxyModel::index (int row, int column, QModelIndex const & parent) const
{
...
QModelIndex TreeProxyModel::parent (QModelIndex const & child) const
{
	return QModelIndex();
}

int TreeProxyModel::rowCount (QModelIndex const & parent) const
{
	return m_map_from_tgt.size();
}

int TreeProxyModel::columnCount (QModelIndex const & parent) const
{
	return m_cmap_from_tgt.size();
}

bool TreeProxyModel::rowInProxy (int row) const
{
...
bool TreeProxyModel::colInProxy (int col) const
{
	return colToSource(col) > -1;
}

int TreeProxyModel::colToSource (int col) const
{
...
int TreeProxyModel::colFromSource (int col) const
{
...
QModelIndex TreeProxyModel::mapToSource (QModelIndex const & proxyIndex) const
{
...
QModelIndex TreeProxyModel::mapFromSource (QModelIndex const & sourceIndex) const
{
...
bool TreeProxyModel::insertRows (int first, int last, QModelIndex const & parent)
{
	return true;
}

bool TreeProxyModel::insertColumns (int first, int last, QModelIndex const & parent)
{
	return true;
}

void TreeProxyModel::insertAllowedColumn (int src_col)
{
...
void TreeProxyModel::removeAllowedColumn (int src_col)
{
	if (src_col >= m_allowed_src_cols.size())
		m_allowed_src_cols.resize(src_col + 1);
...
bool TreeProxyModel::filterAcceptsColumn (int sourceColumn, QModelIndex const & source_parent) const
{
	bool drop = true;
...
bool TreeProxyModel::filterAcceptsRow (int sourceRow, QModelIndex const & /*sourceParent*/) const
{
	bool is_empty = true;
