...
ButtonColumnDelegate::ButtonColumnDelegate(DockManager & mgr, QObject * parent)
	: QStyledItemDelegate(parent)
	, m_dock_mgr(mgr)
{
	btn = new QPushButton("X", 0);
	btn->hide();
	m_dock_mgr.setMouseTracking(true);
	connect(&m_dock_mgr, SIGNAL(entered(QModelIndex)), this, SLOT(cellEntered(QModelIndex)));
...
void ButtonColumnDelegate::cellEntered(const QModelIndex &index)
{
    if(index.model()->headerData(index.column(), Qt::Horizontal, Qt::UserRole) == 1)
    {
        ...
        {
            m_dock_mgr.closePersistentEditor(currentEditedCellIndex);
        }
        m_dock_mgr.openPersistentEditor(index);
	...
	, m_config(g_traceServerName)
{
	...
	setItemDelegateForColumn(e_Column_Close, new ButtonColumnDelegate(*this, this));
