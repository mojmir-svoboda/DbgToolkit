...
{
    bool const res = flt_comparators[m_operator](f, m_value);
    return res;
}

FilteredTime::FilteredTime (QString const & op, uint64_t rhs, QString const & units, bool enabled)
    : m_string(QString::number(rhs))
...
FilteredTime::FilteredTime (QString const & op, uint64_t rhs, QString const & units)
    : m_string(QString::number(rhs))
    , m_time_units_str(units)
    , m_src_value(rhs)
    , m_value(static_cast<float>(m_src_value))
...
bool FilterTime::accept (QModelIndex const & sourceIndex)
{
// 	if (m_data.size() > 0)
// 	{
// 		unsigned long long t;
//         QString qt;
// 		if (!cmd.getString(proto::tag_ctime, qt))
// 			return true;
//         t = qt.toULongLong();
// 
//         bool accepted = true;
...
void FilterTime::onAdd (QString const & op, uint64_t rhs, QString const & units)
{
...
//   if (!found)
//   {
//       QList<QStandardItem *> row_items = add4Col(Qt::Checked, get_tag_name(proto::tag_ctime), op, rhs, units);
//       root->appendRow(row_items);
//   }
