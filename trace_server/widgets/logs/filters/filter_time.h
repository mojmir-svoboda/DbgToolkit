...
struct FilteredTime {
	...
	FilteredTime () { }
	FilteredTime (QString const & op, uint64_t rhs, QString const & units, bool enabled);
	FilteredTime (QString const & op, uint64_t rhs, QString const & units);
...
struct FilterTime : FilterBase
{
	...
	void setupModel ();
	void destroyModel ();
    void append (QString const & op, uint64_t t, QString const & units, bool enabled);
	void appendToWidgets (FilteredTime const & flt);
    void remove (QString const & op, uint64_t s, QString const & u);
...
public slots:
	...
	void onRm ();
	void onAdd ();
    void onAdd (QString const & op, uint64_t rhs, QString const & units);
signals:
